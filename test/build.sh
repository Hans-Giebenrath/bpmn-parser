...
run() {
    if $release; then
        echo time timeout 3s "${CARGO_TARGET_DIR:-./target}"/release/bpmn-parser "$@"
        time timeout 3s "${CARGO_TARGET_DIR:-./target}"/release/bpmn-parser "$@"
    else
        echo time timeout 3s "${CARGO_TARGET_DIR:-./target}"/debug/bpmn-parser "$@"
        time timeout 3s "${CARGO_TARGET_DIR:-./target}"/debug/bpmn-parser "$@"
    fi
}
file_stem=compiled
