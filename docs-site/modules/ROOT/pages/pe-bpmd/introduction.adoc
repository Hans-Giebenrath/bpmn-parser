...
image::generated/pebpmd-introduction.svg[]
...
The primary result of the PE-BPMD analysis is the color coding in the diagram, and the
...
reference the respective paragraph on the TEE page when it exists).
...
capable of decrypting the data and see the plaintext. In my interpretation of these categories,
if one would use a weak implementation of an algorithm, say RSA with keys of 512 bit length, then this
would technically also be "A" since you just need to throw enough dollars at the problem and the
protection is surmounted within a relatively short time frame (less than 3 years).
...
[[sc]]xref:pe-bpmd/secure-channel.adoc[Secure Channel]:: You say where data is protected and where the
...
[[tee]]xref:pe-bpmd/tee.adoc[Trusted Execution Environments]:: CPU extensions which allow to execute code
...
[[mpc]]xref:pe-bpmd/mpc.adoc[Multi-Party Computation (Work In Progress)]:: I am no expert in MPC technology, so read this with
