...
[pe-bpmd PrivateKey in A, SharedSymmKey in B] // <3> <1>
----
...
<3> This could also be used for a plugin system where `pe-bpmd` (a plugin for integrating with PLEAK) would be registered on the CLI when running the tool. This plugin could arbitrarily modify the graph.
