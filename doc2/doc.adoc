...
^ The first letter (in this case `X`) further refines the type of this statement.
   ...
   *G: Complex
   ...
   Boundary Event Interrupting (recommended to indent with two spaces) (`!` used since it terminates the task it is attached to):
     ...
     <!: Compensation
     X!: Cancel
     // not yet supported since I don't know how the XML looks like.
     #!: Multiple
     +!: Multiple Parallel
...
```
OD Regular Display Text -> label "Only specify Display Text once for this edge"

...

OD Regular Display Text <- label // No Display Text here.
```
